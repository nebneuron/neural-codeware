...
int FindCliques(graph_t *ptrGraph, int iMinWeight, int iMaxWeight, int bOnlyMaximal, int iMaxNumCliques, set_t *ptrCliqueList, int iCliqueListLength)
{
    int            iNumCliques;
    ...
    iNumCliques = clique_find_all(ptrGraph, iMinWeight, iMaxWeight, bOnlyMaximal, &localopts);
 ...
 *     prhs[0]      mtxAdj          The input adjacency matrix.  Only the upper-triangular part of this is used.
 *     prhs[1]      iMinWeight      The minimum weight of cliques to find.
 *     prhs[2]      iMaxWeight      The maximum weight of cliques to find.
 *     prhs[3]      bOnlyMaximal    If `false` (i.e., zero), return all cliques; otherwise, return only maximal cliques.
 *     prhs[4]      iMaxNumCliques  The maximum number of cliques to be returned (due to a limitation of Cliquer).
 */
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    /* Retrieve and store the size of the input adjacency matrix for size checking and
     * later use. */
    ...
    if ((iRows != iCols) || !mxIsNumeric(prhs[0]) || mxIsComplex(prhs[0]))
        ...
        mexErrMsgTxt("The fourth argument must be a boolean (`true` or `false`) scalar.");
    ...
    /* These variables are for storing the graph corresponding to the input adjacency
     * matrix, the list of cliques found in that graph, and the return value for this
     * MATLAB function. */
    graph_t  *ptrGraph;                                             /* The graph that we will create from the input matrix. */
    ...
    iNumCliques = FindCliques(ptrGraph, iMinWeight, iMaxWeight, bOnlyMaximal, iMaxNumCliques, arrCliqueList, iMaxNumCliques);
    
    /* We are done with the graph.  Free the memory used to store it. */
    graph_free(ptrGraph);
    
    /* Retrieve the number of cliques returned by the function, which is bounded above by `iMaxNumCliques`. */
